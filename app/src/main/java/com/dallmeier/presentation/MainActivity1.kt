...
import com.dallmeier.myapplication.databinding.ActivityMainBinding
...
class MainActivity1 : Activity() {
    ...
    public override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnWrite.setOnClickListener {
            try {
                if (myTag == null) {
                    Toast.makeText(this, MainActivity1.Companion.ERROR_DETECTED, Toast.LENGTH_LONG).show()
                } else {
                    write(message.text.toString(), myTag)
                    Toast.makeText(this, MainActivity1.Companion.WRITE_SUCCESS, Toast.LENGTH_LONG).show()
                }
            } catch (e: IOException) {
                Toast.makeText(this, MainActivity1.Companion.WRITE_ERROR, Toast.LENGTH_LONG).show()
                e.printStackTrace()
